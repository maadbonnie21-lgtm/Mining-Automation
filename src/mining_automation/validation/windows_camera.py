...
__all__ = [
    ...
    "DISCONNECTED_OK_CLICK_DWELL_SECONDS",
...
DISCONNECTED_OK_CLICK_DWELL_SECONDS = 0.100
...
class RealWindowsCameraApi:
    ...
    def focused_descendant(self, hwnd: int) -> int:
        """Resolve the target thread's existing focused child without activation."""

        result: int = self._calls.focused_descendant(hwnd)
        return result

    def post_key_message(
        self,
        target_hwnd: int,
        virtual_key: int,
        *,
        ...
        """Post one key phase directly to a child HWND without global input."""

        result: bool = self._calls.post_key_message(
            target_hwnd,
            virtual_key,
            key_up=key_up,
            extended=extended,
...
class WindowsCameraControl:
    ...
    def preflight(self) -> CameraPreflightReceipt:
        """Report readiness without activating or otherwise mutating the window.

        Camera validation must never repair focus on the caller's behalf.  An
        activation attempt can restore, resize, or move a Java/AWT RuneLite
        window before the geometry guard gets a chance to reject it.  Require
        the reviewed HWND to be foreground already; later input methods repeat
        this check immediately before dispatch.
        """
    ...
    def click_disconnected_ok(self, x: int, y: int) -> CameraInputReceipt:
        """Click only the reviewed normal disconnect dialog OK point."""

        self._require_reviewed_point(
            "disconnect OK click", x, y, DISCONNECTED_OK_CLIENT_POINT
        )
        self._require_ready()
        self._require_all_control_inputs_released(operation="disconnect OK click")
        screen_point = self._move_to_client_point(x, y)
        self._require_ready()
        self._require_target_at_screen_point("disconnect OK click", screen_point)
        self._require_all_control_inputs_released(operation="disconnect OK click")
        completed_down = 0
        self._left_button_owned = True
        try:
            ...
            if completed_down not in (0, 1):
                raise WindowsCameraError(
                    "Windows returned an invalid disconnect OK left-button-down event count"
                )
            if completed_down == 1:
                self._click_sleeper(DISCONNECTED_OK_CLICK_DWELL_SECONDS)
                self._require_owned_click_point_still_safe(
                    x, y, operation="disconnect OK click"
                )
        finally:
            completed_up = self._release_mouse_button()
        return CameraInputReceipt(
            CameraInputOperation.DISCONNECTED_OK_CLICK,
            requested_events=2,
            completed_events=completed_down + completed_up,
        )
