...
_WM_KEYDOWN: Final[int] = 0x0100
_WM_KEYUP: Final[int] = 0x0101
_MAPVK_VK_TO_VSC: Final[int] = 0
...
def focused_descendant(hwnd: int) -> int:
    ...
    if not _user32.GetGUIThreadInfo(thread_id, ctypes.byref(info)):
        raise OSError("GetGUIThreadInfo failed for target window thread")
    focused = int(info.hwndFocus) if info.hwndFocus else 0
    if focused <= 0 or int(_user32.GetAncestor(focused, _GA_ROOT)) != hwnd:
        raise OSError("target RuneLite thread has no focused descendant")
    return focused
...
def post_key_message(
    ...
    *,
    ...
    """Post one key phase to a resolved child HWND without activation."""

    scan_code = int(_user32.MapVirtualKeyW(virtual_key, _MAPVK_VK_TO_VSC))
    lparam = 1 | (scan_code << 16)
    if extended:
        lparam |= 1 << 24
    if key_up:
        lparam |= (1 << 30) | (1 << 31)
    message = _WM_KEYUP if key_up else _WM_KEYDOWN
    return bool(_user32.PostMessageW(target_hwnd, message, virtual_key, lparam))
