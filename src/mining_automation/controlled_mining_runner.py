...
class ProductionMiningPerceptionEvaluator:
    ...
    def _evaluate_packaged_inventory(
        ...
        from .perception.inventory.retained_iron import (
            retained_iron_count,
            retained_iron_count_from_empty_difference,
        )

        iron_count = retained_iron_count(frame, result)
        if iron_count is None:
            iron_count = retained_iron_count_from_empty_difference(
                frame, result, _INVENTORY_EMPTY_REGION_PATH.read_bytes()
            )
