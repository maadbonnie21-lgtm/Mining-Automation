...
def test_consensus_registration_rejects_no_global_intersection() -> None:
    first = {"sw": _state("sw", True, 0.7, (0, 0, 20, 20))}
    second = {"sw": _state("sw", True, 0.8, (0, 0, 8, 20))}
    third = {"sw": _state("sw", True, 0.9, (12, 0, 8, 20))}
    assert three_rock._choose_consensus_available_registration(
        [
            _candidate("one", first),
            _candidate("two", second),
            _candidate("three", third),
        ]
    ) is None
